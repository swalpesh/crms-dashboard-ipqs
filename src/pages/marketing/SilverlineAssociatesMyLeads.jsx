...
const SilverlineAssociateLeadinfo = () => {
  ...
  const fetchLeads = async () => {
    ...
    try {
        ...
        if(!token) return;
        const response = await fetch(`${API_BASE_URL}/api/saleads/my-leads?lead_status=new`, {
            method: 'GET',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        // TELEMARKETING LOGIC INTEGRATION FOR ASSOCIATE MARKETING
        const isTeleMarketing = empId.startsWith("ATM-IPQS");
        ...
        const response = await fetch(`${API_BASE_URL}/api/saleads`, {
            method: 'POST',
        ...
        if (response.ok) {
            ...
            setTimeout(async () => {
                try {
                    ...
                    // If Telemarketing, alert the Associate Head that it needs assignment
                    if (isTeleMarketing) {
                        notifyTo = "IPQS-E25017"; // Send to Associate Head ID
      ...
      <Box sx={{ maxWidth: 1400, mx: 'auto', display: 'flex', flexDirection: 'column', gap: 3 }}>
        <Box sx={{ ...glassPanel, p: 3, display: 'flex', flexDirection: { xs: 'column', md: 'row' }, justifyContent: 'space-between', alignItems: 'center', gap: 3 }}>
          <Box sx={{ textAlign: { xs: 'center', md: 'left' } }}>
            ...
            <Typography variant="body2" sx={{ color: '#a0a0c0' }}>Manage Silverline Associates leads</Typography>
          </Box>
          {!isMobile && (
...
export default SilverlineAssociateLeadinfo;
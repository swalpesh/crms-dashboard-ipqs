...
const YKAssosiatesDashboard = () => {
  ...
  const isHead = authRole === 'Y-k-Enterprises-Associates-Head';
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      // 1. Fetch Hot Leads
      try {
        const res = await fetch(`${API_BASE_URL}/api/ykaleads/hot-leads`, { headers });
        if (res.ok) {
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/api/ykaleads/employees-revenue`, { headers });
        if (res.ok) {
          const result = await res.json();
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/api/ykaleads/new-assigned-summary`, { headers });
        if (res.ok) {
          const result = await res.json();
          if (isHead) {
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/api/ykaleads/sales-funnel`, { headers });
        if (res.ok) {
          const result = await res.json();
...
export default YKAssosiatesDashboard;
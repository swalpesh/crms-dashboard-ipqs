...
const SilverlineAssociatesLeadManager = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const queueRes = await fetch(`${API_BASE_URL}/api/saleads/silverlineassociates/all-leads`, { method: 'GET', headers });
        // 2. Fetch Calendar Details
        const calendarRes = await fetch(`${API_BASE_URL}/api/saleads/silverlineassociates/visit-details`, { method: 'GET', headers });
  ...
  const handleCompleteAction = async () => {
    ...
    if (!selectedEmployee) return setToast({ open: true, message: 'Please select a Silverline person.', severity: 'warning' });
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/saleads/assign`, {
      ...
      setTimeout(async () => {
        try {
            const notificationPayload = {
                ...
                message: `You have been assigned a new Silverline Department visit for lead: ${currentLead.lead_name || currentLead.company_name}`
            };
...
export default SilverlineAssociatesLeadManager;
...
export default function FollowUps() {
  ...
  const hideAssignCol = role === "tele-marketing-head" || role === "tele-marketing-employee";
  ...
  async function fetchTodayFollowups() {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/leads/my-leads/today-followups`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(json?.message || "Failed to fetch today's follow-ups");
      const arr = Array.isArray(json?.leads) ? json.leads : [];
      setFollowUps(arr.map(mapLead));
    } catch (e) {
      console.error(e);
      setFollowUps([]);
    } finally {
  ...
  // Assign handlers (UI placeholder)
  ...
  // Reschedule handlers (API status follow-up)
  const openReschedule = (lead) => {
    setReschedLead(lead);
    setReschedNote(lead?.follow?.note || "");
    // follow_up_date from API can be ISO — normalize to yyyy-mm-dd input value
    const d = lead?.follow?.date ? new Date(lead.follow.date) : null;
    const ymd =
      d &&
  ...
  const headerColSpan = showAssignCol ? 8 : 7;
        ...
        <Box sx={{ pb: 1, flexShrink: 0 }}>
          <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1 }}>
            <Stack direction="row" spacing={1} alignItems="center">
              ...
              {loading && <CircularProgress size={14} sx={{ ml: 0.75 }} />}
            </Stack>
            <Button size="small" onClick={fetchTodayFollowups}>
              Refresh
            </Button>
          </Stack>

// src/pages/marketing/TechnicalMyTeam.jsx
import {
  Avatar,
  Box,
  Button,
...
/* ------------------------------ page ------------------------------- */
export default function TechnicalMyTeam() {
  ...
  async function fetchTeam() {
    ...
    try {
      ...
      const employees =
        json.employees || json.technicalEmployees || json.data?.employees || [];
      const unassignedLeads =
        json.unassigned_leads ||
        json.unassignedLeads ||
        json.data?.unassigned_leads ||
        [];
  ...
  async function apiAssignLead(leadId, empId) {
    const token = getToken();
    const res = await fetch(`${API_BASE_URL}/api/tleads/assign`, {
      method: "PATCH",
      headers: {
  ...
  const goToLead = (id) =>
    navigate(`/marketing/lead/${encodeURIComponent(id)}`);
      ...
      >
        ...
        <DialogActions sx={{ p: 2 }}>
          ...
          <Button
            ...
            onClick={() => {
              if (confirmBack) doServerMove(confirmBack);
              setConfirmBack(null);
            }}
          >
            Move Back

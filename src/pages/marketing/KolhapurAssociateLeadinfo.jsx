...
const KolhapurAssociateLeadinfo = () => {
  ...
  const fetchLeads = async () => {
    ...
    try {
        ...
        if(!token) return;
        const response = await fetch(`${API_BASE_URL}/api/kaleads/my-leads?lead_status=new`, {
            method: 'GET',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/kaleads`, {
            method: 'POST',
      ...
      <Box sx={{ maxWidth: 1400, mx: 'auto', display: 'flex', flexDirection: 'column', gap: 3 }}>
        <Box sx={{ ...glassPanel, p: 3, display: 'flex', flexDirection: { xs: 'column', md: 'row' }, justifyContent: 'space-between', alignItems: 'center', gap: 3 }}>
          <Box sx={{ textAlign: { xs: 'center', md: 'left' } }}>
            ...
            <Typography variant="body2" sx={{ color: '#a0a0c0' }}>Manage Kolhapur Associates leads</Typography>
          </Box>
          {!isMobile && (
...
export default KolhapurAssociateLeadinfo;
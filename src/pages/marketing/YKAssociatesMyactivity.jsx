...
const YKAssociatesMyactivity = () => {
  ...
  const isMarketingHead = authRole === 'Y-k-Enterprises-Associates-Head';
  ...
  const [scheduleReason, setScheduleReason] = useState('YK Enterprises Visit Rescheduled.'); 
  ...
  const fetchAllData = async () => {
    ...
    try {
      const [unSchedRes, schedRes, compRes, statusRes] = await Promise.all([
        fetch(`${API_BASE_URL}/api/ykaleads/unscheduled-leads`, { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch(`${API_BASE_URL}/api/ykaleads/scheduled-visits`, { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch(`${API_BASE_URL}/api/ykaleads/completed-visits`, { headers: { 'Authorization': `Bearer ${token}` } }),
  ...
  const handleConfirmSchedule = async () => {
    ...
    try {
      ...
      if (isRescheduleMode) {
        // Hit Reschedule API
        apiUrl = `${API_BASE_URL}/api/ykaleads/reschedule-visit`;
        payload = {
        ...
        apiUrl = `${API_BASE_URL}/api/ykaleads/assign`;
        payload = {
          lead_id: selectedLead.lead_id,
  ...
  const handleStartVisit = async (lead) => {
    try {
      ...
      const visitPromise = fetch(`${API_BASE_URL}/api/ykaleads/visit-status`, {
        method: 'PATCH',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
  ...
  const submitEndVisit = async () => {
    ...
    try {
      ...
      promises.push(
        fetch(`${API_BASE_URL}/api/ykaleads/visit-status`, {
          method: 'PATCH',
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      // 2. Change Lead Stage
      promises.push(
        fetch(`${API_BASE_URL}/api/ykaleads/change-stage`, {
          method: 'PATCH',
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({
            lead_id: endVisitLead.lead_id,
      ...
      if (isSolutions) {
          ...
          try {
              await fetch(`${API_BASE_URL}/api/ykaleads/${endVisitLead.lead_id}`, {
                  method: 'PUT',
                  headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
      ...
      setTimeout(async () => {
        try {
          ...
          const notificationPayload = {
            ...
            message: isSolutions 
              ? `New lead transferred from YK Enterprises for solutions. Lead ID: ${endVisitLead.lead_id} (${endVisitLead.company_name || endVisitLead.lead_name})`
              : `YK Enterprises Lead Transferred to ${deptName}. Lead ID: ${endVisitLead.lead_id} (${endVisitLead.company_name || endVisitLead.lead_name}) by ${currentUser?.username || 'Employee'}.`
          };
...
export default YKAssociatesMyactivity;
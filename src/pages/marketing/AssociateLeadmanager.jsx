...
const AssociateLeadManager = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const queueRes = await fetch(`${API_BASE_URL}/api/aleads/associatemarketing/all-leads`, { method: 'GET', headers });
        // 2. Fetch Calendar Details
        const calendarRes = await fetch(`${API_BASE_URL}/api/aleads/associatemarketing/visit-details`, { method: 'GET', headers });
        ...
        // Populate Employees (Calculate load based on leads array length)
        const employeeData = (queueData.employees || []).map(emp => ({
            ...emp,
            total_leads: emp.leads ? emp.leads.length : 0 
        }));
  ...
  const handleCompleteAction = async () => {
    ...
    if (!selectedEmployee) return setToast({ open: true, message: 'Please select a Associate person.', severity: 'warning' });
    ...
    try {
      ...
      // 2. Construct Payload according to API requirement
      ...
      const payload = {
        ...
        associate_visit_date: visitDate,
        associate_visit_time: formattedTime,
        associate_visit_priority: priority,
      ...
      // 3. Call Assign API (PATCH)
      const response = await fetch(`${API_BASE_URL}/api/aleads/assign`, {
      ...
      setEmployees(prev => prev.map(emp => {
          if(emp.employee_id === currentEmployee.employee_id) {
              return { ...emp, total_leads: (emp.total_leads || 0) + 1 };
          }
          return emp;
      }));
      ...
      setTimeout(async () => {
        try {
            const notificationPayload = {
                ...
                message: `You have been assigned a new Associate visit for lead: ${currentLead.lead_name || currentLead.company_name}`
            };
  ...
  const AssignSection = () => (
    <Box sx={glassPanelStyle}>
      ...
      {/* Employee List */}
      <Box>
        <Typography variant="caption" color="rgba(255,255,255,0.6)" mb={1} display="block" fontWeight={600}>Select Associate Person</Typography>
        <Box sx={{ bgcolor: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', p: 2 }}>
          ...
          <Box sx={{ ...inputStyle, display: 'flex', alignItems: 'center', gap: 1, mb: 2, py: 1 }}>
            ...
            <InputBase placeholder="Search visit person..." sx={{ color: '#fff', fontSize: '0.9rem', width: '100%' }} value={techSearch} onChange={(e) => setTechSearch(e.target.value)} />
          </Box>
          ...
          <Stack spacing={1} sx={{ maxHeight: '200px', overflowY: 'auto', pr: 0.5, '&::-webkit-scrollbar': { width: '4px' }, '&::-webkit-scrollbar-thumb': { bgcolor: 'rgba(255,255,255,0.1)', borderRadius: '4px' } }}>
            {filteredTechs.map((emp, i) => {
              const status = getLoadStatus(emp.total_leads); // Uses the calculated total_leads
    ...
    <Box sx={pageStyle}>
      ...
      <Box sx={{ position: 'relative', zIndex: 1, maxWidth: '1400px', mx: 'auto', animation: `${slideUpFade} 0.8s ease-out` }}>
        ...
        <Typography variant="body1" color="rgba(255,255,255,0.6)" mb={5}>
          Manage team schedules and assign Associate visits
        </Typography>
...
export default AssociateLeadManager;
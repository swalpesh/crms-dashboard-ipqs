// src/pages/marketing/CorporateLeads.jsx
...
export default function CorporateLeads() {
  ...
  async function fetchLeadsBy(status) {
    const token = getToken();
    if (!token) return [];
    const url = `${API_BASE_URL}/api/cleads/my-leads?lead_status=${encodeURIComponent(status)}`;
  ...
  async function changeStage(leadId, newStage, reason) {
    ...
    const res = await fetch(`${API_BASE_URL}/api/cleads/change-stage`, {
      ...
      body: JSON.stringify({
        lead_id: leadId,
        new_lead_stage: newStage, // e.g., "Corporate-Marketing"
        reason: reason || "",
      }),
    });
    const json = await res.json().catch(() => ({}));
  ...
  async function createLead(fd) {
    const token = getToken();
    const res = await fetch(`${API_BASE_URL}/api/cleads`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
      body: fd,
    });
  ...
  const saveLead = async () => {
    ...
    try {
      ...
      fd.append("lead_stage", "Corporate-Marketing");
          ...
          <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1 }}>
            {isMdUp && (
              <Stack direction="row" spacing={1} alignItems="center">
                <Typography variant="h5" fontWeight={700}>Corporate Leads</Typography>

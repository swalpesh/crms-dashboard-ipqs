...
const LeadManager = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const queueRes = await fetch(`${API_BASE_URL}/api/fleads/fieldmarketing/all-leads`, { method: 'GET', headers });
        // 2. Fetch Calendar Details
        const calendarRes = await fetch(`${API_BASE_URL}/api/fleads/fieldmarketing/visit-details`, { method: 'GET', headers });
  ...
  const handleCompleteAction = async () => {
    ...
    if (!selectedEmployee) return setToast({ open: true, message: 'Please select a Field person.', severity: 'warning' });
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/fleads/assign`, {
      ...
      setTimeout(async () => {
        try {
            const notificationPayload = {
                ...
                message: `You have been assigned a new field visit for lead: ${currentLead.lead_name || currentLead.company_name}`
            };
  ...
  const AssignSection = () => (
    <Box sx={glassPanelStyle}>
      ...
      {/* Employee List */}
      <Box>
        <Typography variant="caption" color="rgba(255,255,255,0.6)" mb={1} display="block" fontWeight={600}>Select Field Person</Typography>
    ...
    <Box sx={pageStyle}>
      ...
      <Box sx={{ position: 'relative', zIndex: 1, maxWidth: '1400px', mx: 'auto', animation: `${slideUpFade} 0.8s ease-out` }}>
        ...
        <Typography variant="body1" color="rgba(255,255,255,0.6)" mb={5}>
          Manage team schedules and assign Field visits
        </Typography>

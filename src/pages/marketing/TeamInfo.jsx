...
const LeadItem = ({ lead, onMapClick }) => {
  ...
  const getStatusColor = (status) => {
    ...
    if (s === 'progress' || s === 'started') return themeColors.accentBlue;
    return themeColors.textSecondary; // Grey for pending/new
  };

  const statusColor = getStatusColor(lead.lead_status);
  const isStarted = lead.lead_status === 'progress' || lead.lead_status === 'completed';
  const isCompleted = lead.lead_status === 'completed';

  const timelineData = [
    { label: "Assigned", time: formatDateTime(lead.technical_visit_date, lead.technical_visit_time), completed: true, showMap: false },
    { label: "Visit Started", time: isStarted ? "On Site" : "Pending", completed: isStarted, showMap: isStarted && lead.technical_visit_start_location },
    { label: "Completed", time: isCompleted && lead.technical_visit_complete_time ? formatDateTime(lead.technical_visit_complete_time) : "Pending", completed: isCompleted, showMap: false }
  ];
    ...
    <Box sx={{ 
      ...
      <Box sx={{ p: 2, cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }} onClick={() => setExpanded(!expanded)}>
        ...
        <Stack direction="row" spacing={1} alignItems="center">
          <Chip 
            label={lead.lead_visit_status || lead.lead_status} 
            size="small" 
            sx={{ 
                  ...
                  <Box flexGrow={1}>
                    <Stack direction="row" justifyContent="space-between" alignItems="center">
                        ...
                        {event.showMap && (
                            <IconButton 
                                size="small" 
                                onClick={(e) => { e.stopPropagation(); onMapClick(lead.technical_visit_start_location); }}
                                sx={{ 
                                    color: '#fff', 
    ...
    const fetchTeamData = async () => {
      ...
      try {
        ...
        const endpoint = tab === 0 
            ? `${API_BASE_URL}/api/tleads/technicalteam/todays-all-visits`
            : `${API_BASE_URL}/api/tleads/technicalteam/all-leads`;

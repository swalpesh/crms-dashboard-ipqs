...
const KolhapurAssociatesDashboard = () => {
  ...
  const isHead = authRole === 'Kolhapur-Associates-Head';
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      // 1. Fetch Hot Leads
      try {
        const res = await fetch(`${API_BASE_URL}/api/kaleads/hot-leads`, { headers });
        if (res.ok) {
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/api/kaleads/employees-revenue`, { headers });
        if (res.ok) {
          const result = await res.json();
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/api/kaleads/new-assigned-summary`, { headers });
        if (res.ok) {
          const result = await res.json();
          if (isHead) {
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/api/kaleads/sales-funnel`, { headers });
        if (res.ok) {
          const result = await res.json();
...
export default KolhapurAssociatesDashboard;
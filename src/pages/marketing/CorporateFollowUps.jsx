// src/pages/marketing/CorporateFollowUps.jsx
...
// ---------------- Component ----------------
export default function CorporateFollowUps() {
  ...
  async function fetchFollowUps() {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/cleads/my-leads/today-followups`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      const json = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(json?.message || "Failed to fetch follow-ups");

      const arr = Array.isArray(json?.leads)
        ? json.leads.map(mapLead)
        : [];
      setFollowUps(arr);
    } catch (err) {
  ...
  async function patchStatus(leadId, payload) {
    const token = getToken();
    const res = await fetch(`${API_BASE_URL}/api/cleads/${leadId}/status`, {
      method: "PATCH",
      headers: {
  ...
  async function revertLead(leadId, reason) {
    const token = getToken();
    const res = await fetch(`${API_BASE_URL}/api/cleads/${leadId}/revert`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
  ...
  async function assignToDepartment(leadId, department, reason) {
    const token = getToken();
    const res = await fetch(`${API_BASE_URL}/api/cleads/change-stage`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
  ...
  const allContacts = Array.from(
    new Set(followUps.map((l) => l.contact))
  ).filter(Boolean);
  const allLeadTypes = Array.from(
    new Set(followUps.map((l) => l.leadType))
  ).filter(Boolean);
  const allCompanies = Array.from(
    new Set(followUps.map((l) => l.company))
  ).filter(Boolean);
  const allLocations = Array.from(
    new Set(followUps.map((l) => l.location))
  ).filter(Boolean);
  ...
  const saveReschedule = async () => {
    if (!reschedLead || !reschedNote.trim() || !reschedDate || !reschedTime)
      return;
  ...
  return (
    ...
    >
          ...
          >
            <Stack direction="row" spacing={1} alignItems="center">
              <Typography variant="h5" fontWeight={700}>
                Corporate Marketing Follow-ups
              </Typography>
          ...
          >
            ...
            <TableBody>
              {loading ? (
                ...
                filtered.map((lead) => {
                  const nextDate = lead?.follow?.date
                    ? fmtDate(lead.follow.date)
                    : "-";
      ...
      <Dialog open={assignOpen} onClose={() => setAssignOpen(false)} fullWidth>
        ...
        <DialogActions>
          ...
          <Button
            variant="contained"
            onClick={saveAssign}
            disabled={!assignDept || !assignReason.trim() || assignLoading}
          >
      ...
      {/* Back Dialog */}
      <Dialog open={backOpen} onClose={() => setBackOpen(false)} fullWidth>
        ...
        <DialogActions>
          ...
          <Button
            variant="contained"
            onClick={saveBack}
            disabled={!backReason.trim() || backLoading}
          >
      ...
      {/* Reschedule Dialog */}
      <Dialog open={reschedOpen} onClose={() => setReschedOpen(false)} fullWidth>

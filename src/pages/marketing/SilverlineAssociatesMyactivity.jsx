...
// --- MAIN COMPONENT ---
const SilverlineAssociatesMyactivity = () => {
  ...
  const isMarketingHead = authRole === 'Silverline-Associates-Head';
  ...
  const [completedLeadsData, setCompletedLeadsData] = useState([]); 
  ...
  const [scheduleReason, setScheduleReason] = useState('Silverline Associates Visit Rescheduled.'); 
  ...
  const fetchAllData = async () => {
    ...
    try {
      const [unSchedRes, schedRes, compRes, statusRes] = await Promise.all([
        fetch(`${API_BASE_URL}/api/saleads/unscheduled-leads`, { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch(`${API_BASE_URL}/api/saleads/scheduled-visits`, { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch(`${API_BASE_URL}/api/saleads/completed-visits`, { headers: { 'Authorization': `Bearer ${token}` } }),
  ...
  const handleOpenSchedule = (lead, isRescheduling = false) => { 
    ...
    setScheduleDate(lead.silverline_associate_visit_date ? lead.silverline_associate_visit_date.split('T')[0] : '');
    setScheduleTime(lead.silverline_associate_visit_time || '');
    setSchedulePriority(lead.silverline_associate_visit_priority || 'Medium');
    setIsRescheduleMode(isRescheduling);
    setScheduleReason(isRescheduling ? 'Silverline Associates Visit Rescheduled.' : '');
    setOpenModal(true); 
  };
  ...
  const handleConfirmSchedule = async () => {
    ...
    try {
      ...
      if (isRescheduleMode) {
        // Hit Reschedule API
        apiUrl = `${API_BASE_URL}/api/saleads/reschedule-visit`;
        payload = {
        ...
        apiUrl = `${API_BASE_URL}/api/saleads/assign`;
        payload = {
          lead_id: selectedLead.lead_id,
          assigned_employee: currentUser?.employee_id || selectedLead.assigned_employee,
          silverline_associate_visit_date: scheduleDate,
          silverline_associate_visit_time: formattedTime,
          silverline_associate_visit_priority: schedulePriority,
          silverline_associate_visit_type: "Specific",
          reason: "Lead Scheduled for Silverline Associates Visit"
        };
      }
      ...
      // Trigger Notification to specific Corporate Marketing Head
      setTimeout(async () => {
        try {
          const notificationPayload = {
            to_emp_id: "IPQS-H25009",
  ...
  // --- START VISIT ACTION (UPDATED WITH REIMBURSEMENT TRIGGER) ---
  const handleStartVisit = async (lead) => {
    try {
      ...
      const visitPromise = fetch(`${API_BASE_URL}/api/saleads/visit-status`, {
        method: 'PATCH',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
      ...
      setScheduledLeads(prevLeads => prevLeads.map(l => 
        l.lead_id === lead.lead_id ? { ...l, silverline_associate_lead_visit_status: 'Started' } : l
      ));

      setToast({ open: true, message: 'Visit started successfully!', severity: 'success' });

      // Trigger Notification to Corporate Marketing Head
      setTimeout(async () => {
        try {
          const notificationPayload = {
            to_emp_id: "IPQS-H25009",
  ...
  // --- END VISIT & TRANSFER ACTION (UPDATED WITH REIMBURSEMENT TRIGGER) ---
  const submitEndVisit = async () => {
    ...
    try {
      ...
      promises.push(
        fetch(`${API_BASE_URL}/api/saleads/visit-status`, {
          method: 'PATCH',
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      // 2. Change Lead Stage
      promises.push(
        fetch(`${API_BASE_URL}/api/saleads/change-stage`, {
          method: 'PATCH',
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({
            lead_id: endVisitLead.lead_id,
      ...
      // 4. FORCE OVERRIDE: Direct DB Update for Solutions Team
      if (isSolutions) {
      ...
      setScheduledLeads(prevLeads => prevLeads.map(l => 
        l.lead_id === endVisitLead.lead_id ? { 
            ...l, 
            silverline_associate_lead_visit_status: 'Completed', 
      ...
      setTimeout(async () => {
        try {
          ...
          const notificationPayload = {
            ...
            message: isSolutions 
              ? `New lead transferred from Silverline Associates for solutions. Lead ID: ${endVisitLead.lead_id} (${endVisitLead.company_name || endVisitLead.lead_name})`
              : `Silverline Associates Lead Transferred to ${deptName}. Lead ID: ${endVisitLead.lead_id} (${endVisitLead.company_name || endVisitLead.lead_name}) by ${currentUser?.username || 'Employee'}.`
          };
  ...
  const displayedScheduledLeads = useMemo(() => {
    return scheduledLeads.filter((lead) => {
      ...
      if (!lead.silverline_associate_visit_date || !lead.silverline_associate_visit_date.startsWith(activeFilterDate)) return false;

      // 2. Filter by selected Status Filter
      const currentStatus = lead.silverline_associate_lead_visit_status || 'Pending';
  ...
  const todaysTotalLeads = useMemo(() => {
    ...
    return scheduledLeads.filter((lead) => lead.silverline_associate_visit_date && lead.silverline_associate_visit_date.startsWith(todayStr));
  }, [scheduledLeads]);

  const completedCount = todaysTotalLeads.filter(l => l.silverline_associate_lead_visit_status === 'Completed').length;
      ...
      <Box sx={{ maxWidth: '850px', mx: 'auto', pb: 5, position: 'relative', zIndex: 2 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            ...
            {loadingScheduled ? (
              ...
              displayedScheduledLeads.map((lead) => (
                <ActivityCard 
                  key={lead.lead_id}
                  visitStatus={lead.silverline_associate_lead_visit_status} 
                  time={formatTime(lead.silverline_associate_visit_time)} 
                  ...
                  disableStart={!lead.silverline_associate_visit_date?.startsWith(todayStr)} // Disable if not today
                  inTrip={inTrip} 
        ...
        {activeTab === 2 && (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            ...
            {/* Filter Dropdown (Only for Corporate-Marketing-Head) */}
            {isMarketingHead && (
                 ...
                 <Box key={lead.lead_id} sx={{ ...styles.glassCard, mb: 2 }}>
                   ...
                   <Grid container spacing={2} sx={{ mb: 2 }}>
                     <Grid item xs={12} sm={4}>
                       <InfoItem icon={<Clock size={18} />} label="Visit Time" value={formatTime(lead.silverline_associate_visit_time || lead.visit_time)} />
                     </Grid>
                     <Grid item xs={12} sm={4}>
                       <InfoItem icon={<CalendarPlus size={18} />} label="Visit Date" value={lead.silverline_associate_visit_date || lead.visit_date ? new Date(lead.silverline_associate_visit_date || lead.visit_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : "Date Not Available"} />
                     </Grid>
                     <Grid item xs={12} sm={4}>
                       <InfoItem icon={<MapPin size={18} />} label="Start Location" value={lead.silverline_associate_visit_start_location || lead.start_location || "Coordinates Captured"} />
                     </Grid>
                   </Grid>
...
export default SilverlineAssociatesMyactivity;
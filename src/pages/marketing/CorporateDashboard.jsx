...
const CorporateDashboard = () => {
  ...
  const isHead = authRole === 'Corporate-Marketing-Head';
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      // 1. Fetch Hot Leads
      try {
        const res = await fetch(`${API_BASE_URL}/api/cleads/hot-leads`, { headers });
        if (res.ok) {
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/api/cleads/employees-revenue`, { headers });
        if (res.ok) {
          const result = await res.json();
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/api/cleads/new-assigned-summary`, { headers });
        if (res.ok) {
          const result = await res.json();
          if (isHead) {
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/api/cleads/sales-funnel`, { headers });
        if (res.ok) {
          const result = await res.json();

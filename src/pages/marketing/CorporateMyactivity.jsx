...
const CoporateMyactivity = () => {
  ...
  const isMarketingHead = authRole === 'Corporate-Marketing-Head';
  ...
  const [scheduleReason, setScheduleReason] = useState('Corporate Visit Rescheduled.'); 
  ...
  const fetchAllData = async () => {
    ...
    try {
      const [unSchedRes, schedRes, compRes, statusRes] = await Promise.all([
        fetch(`${API_BASE_URL}/api/cleads/unscheduled-leads`, { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch(`${API_BASE_URL}/api/cleads/scheduled-visits`, { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch(`${API_BASE_URL}/api/cleads/completed-visits`, { headers: { 'Authorization': `Bearer ${token}` } }),
  ...
  const handleOpenSchedule = (lead, isRescheduling = false) => { 
    ...
    setScheduleDate(lead.corporate_visit_date ? lead.corporate_visit_date.split('T')[0] : '');
    setScheduleTime(lead.corporate_visit_time || '');
    setSchedulePriority(lead.corporate_visit_priority || 'Medium');
    setIsRescheduleMode(isRescheduling);
    setScheduleReason(isRescheduling ? 'Corporate Visit Rescheduled.' : '');
    setOpenModal(true); 
  };
  ...
  const handleConfirmSchedule = async () => {
    ...
    try {
      ...
      if (isRescheduleMode) {
        // Hit Reschedule API
        apiUrl = `${API_BASE_URL}/api/cleads/reschedule-visit`;
        payload = {
        ...
        apiUrl = `${API_BASE_URL}/api/cleads/assign`;
        payload = {
          lead_id: selectedLead.lead_id,
          assigned_employee: currentUser?.employee_id || selectedLead.assigned_employee,
          corporate_visit_date: scheduleDate,
          corporate_visit_time: formattedTime,
          corporate_visit_priority: schedulePriority,
          corporate_visit_type: "Specific",
          reason: "Lead Scheduled for Corporate Visit"
        };
      }
  ...
  const handleStartVisit = async (lead) => {
    try {
      ...
      const visitPromise = fetch(`${API_BASE_URL}/api/cleads/visit-status`, {
        method: 'PATCH',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
      ...
      setScheduledLeads(prevLeads => prevLeads.map(l => 
        l.lead_id === lead.lead_id ? { ...l, corporate_lead_visit_status: 'Started' } : l
      ));
  ...
  const submitEndVisit = async () => {
    ...
    try {
      ...
      promises.push(
        fetch(`${API_BASE_URL}/api/cleads/visit-status`, {
          method: 'PATCH',
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      // 2. Change Lead Stage
      promises.push(
        fetch(`${API_BASE_URL}/api/cleads/change-stage`, {
          method: 'PATCH',
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({
            lead_id: endVisitLead.lead_id,
  ...
  const displayedScheduledLeads = useMemo(() => {
    return scheduledLeads.filter((lead) => {
      ...
      if (!lead.corporate_visit_date || !lead.corporate_visit_date.startsWith(activeFilterDate)) return false;

      // 2. Filter by selected Status Filter
      const currentStatus = lead.corporate_lead_visit_status || 'Pending';
  ...
  const todaysTotalLeads = useMemo(() => {
    ...
    return scheduledLeads.filter((lead) => lead.corporate_visit_date && lead.corporate_visit_date.startsWith(todayStr));
  }, [scheduledLeads]);

  const completedCount = todaysTotalLeads.filter(l => l.corporate_lead_visit_status === 'Completed').length;
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            ...
            {loadingScheduled ? (
              ...
              displayedScheduledLeads.map((lead) => (
                <ActivityCard 
                  key={lead.lead_id}
                  visitStatus={lead.corporate_lead_visit_status} 
                  time={formatTime(lead.corporate_visit_time)} 
                  ...
                  disableStart={!lead.corporate_visit_date?.startsWith(todayStr)} // Disable if not today
                  inTrip={inTrip} 
                 ...
                 <Box key={lead.lead_id} sx={{ ...styles.glassCard, mb: 2 }}>
                   ...
                   <Grid container spacing={2} sx={{ mb: 2 }}>
                     <Grid item xs={12} sm={4}>
                       <InfoItem icon={<Clock size={18} />} label="Visit Time" value={formatTime(lead.corporate_visit_time || lead.visit_time)} />
                     </Grid>
                     <Grid item xs={12} sm={4}>
                       <InfoItem icon={<CalendarPlus size={18} />} label="Visit Date" value={lead.corporate_visit_date || lead.visit_date ? new Date(lead.corporate_visit_date || lead.visit_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : "Date Not Available"} />
                     </Grid>
                     <Grid item xs={12} sm={4}>
                       <InfoItem icon={<MapPin size={18} />} label="Start Location" value={lead.corporate_visit_start_location || lead.start_location || "Coordinates Captured"} />
                     </Grid>
                   </Grid>
...
export default CoporateMyactivity;
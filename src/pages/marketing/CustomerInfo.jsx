...
const FieldRow = ({ label, value, icon, isLink, isUser, isWarning, onClick }) => (
  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: { xs: 2, sm: 4 }, fontSize: '0.95rem' }}>
    ...
    >
      {icon && <Box sx={{ pt: '1px' }}>{icon}</Box>}
      {isUser && value !== 'Unassigned' && (
        <Avatar 
          sx={{ width: 22, height: 22, fontSize: 10, mt: '1px' }} 
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={(stage === 'Solutions-Team' || isQTeamHead) ? 12 : 6}>
            <Box sx={{ ...glassCardStyle, height: '100%' }}>
              <Typography variant="h6" sx={{ borderBottom: themeColors.glassBorder, pb: 2, mb: 3, color: themeColors.textSecondary, display: 'flex', alignItems: 'center', gap: 1 }}>

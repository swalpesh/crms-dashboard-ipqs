...
// Helper to check if current user is Quotation Team Head
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
          {files.map((file, idx) => {
            ...
            return (
              <Box 
                key={idx} 
                sx={{ display: 'flex', alignItems: 'center', gap: 1, bgcolor: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 2, px: 1.5, py: 1, cursor: 'pointer', '&:hover': { borderColor: themeColors.accent } }} 
                // ✅ FIX APPLIED HERE: Prioritizing the smart file_url generated by the backend
                onClick={() => window.open(file.file_url || `${API_BASE_URL}/${file.file_path}`, '_blank')}
              >
...
const CustomerInfo = () => {
  ...
  const isQTeamHead = isQuotationTeamHead(); // Role Check
  ...
  // --- Handlers for Quotation Action ---
  const handleCreateQuotationClick = () => {
    // Pass the lead data to quotation builder using State routing
    navigate("/marketing/quotation-builder", { state: { lead: leadData } });
  };
    ...
    <Box sx={{ minHeight: '100vh', background: themeColors.bgGradient, color: themeColors.textPrimary, fontFamily: "'Inter', sans-serif", p: { xs: 2, md: 4 } }}>
      <Box sx={{ maxWidth: '1200px', mx: 'auto', pb: 5 }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3, alignItems: 'center' }}>
          ...
          {stage === 'Solutions-Team' && !isQTeamHead && (
            <Box sx={{ display: 'flex', gap: 2 }}>
              <Button variant="contained" onClick={() => setSolutionOpen(true)} sx={{ bgcolor: themeColors.success, '&:hover': { bgcolor: '#00a884' }, textTransform: 'none', fontWeight: 600, px: 3 }}>Provide Solution</Button>
              <Button variant="contained" onClick={() => setTransferOpen(true)} sx={{ bgcolor: themeColors.blue, '&:hover': { bgcolor: '#0062cc' }, textTransform: 'none', fontWeight: 600, px: 4 }}>Transfer</Button>
            </Box>
          )}

          {/* Quotation Team Actions */}
          {isQTeamHead && stage === 'Quotation-Team' && (
            <Button 
                variant="contained" 
                ...
                sx={{ 
                  ...
                  '&:hover': { bgcolor: themeColors.accentGlow }, 
                  textTransform: 'none', 
                  fontWeight: 600, 
                  px: 3 
                }}
        ...
        <Grid container spacing={3}>
          {/* Grid Size logic: If QTeam Head, it spans full width. If Sol Team, it spans full width. Otherwise 50% */}
          <Grid item xs={12} md={(stage === 'Solutions-Team' || isQTeamHead) ? 12 : 6}>
          ...
          {/* Conditional Rendering: Hide Internal Tracking if stage is Solutions-Team OR if Quotation Team Head */}
          {stage !== 'Solutions-Team' && !isQTeamHead && (
                                  ...
                                  <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                                    {msg.attachments.map(att => (
                                      <Chip 
                                        ...
                                        // ✅ FIX APPLIED HERE: Use the smart file_url for discussions as well
                                        onClick={() => window.open(att.file_url || `${API_BASE_URL}/${att.file_path}`, '_blank')}
                                      />
                                    ))}
                                  </Box>
                                )}

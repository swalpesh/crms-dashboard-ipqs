// src/pages/marketing/TechnicalLeads.jsx
...
/* ================================ main page ================================ */
export default function TechnicalLeads() {
    ...
    return currentList.filter((l) => {
      const matchesSearch =
        ...
        [
          ...
          l.leadType, l.location, l?.follow?.date, l?.follow?.time, l?.follow?.note
        ]
          .filter(Boolean)
  ...
  const clearFilters = () => {
    ...
    setPersonFilter(""); setCompanyFilter("");
    setLocationFilter(""); setLeadTypeFilter("");
  };
  ...
  async function fetchLeadsBy(status) {
    const token = getToken();
    if (!token) return [];
    const url = `${API_BASE_URL}/api/tleads/my-leads?lead_status=${encodeURIComponent(status)}`;
  ...
  async function changeStage(leadId, newStage, reason) {
    ...
    const res = await fetch(`${API_BASE_URL}/api/tleads/change-stage`, {
      ...
      body: JSON.stringify({
        lead_id: leadId,
        new_lead_stage: newStage, // e.g., "Technical-Team"
        reason: reason || "",
      }),
    });
    const json = await res.json().catch(() => ({}));
  ...
  async function createLead(fd) {
    const token = getToken();
    const res = await fetch(`${API_BASE_URL}/api/tleads`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
      body: fd,
    });
  ...
  const saveLead = async () => {
    ...
    try {
      ...
      fd.append("lead_stage", "Technical-Team");
    ...
    <Box sx={{ p: { xs: 0.75, md: 1.25 }, height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <Paper elevation={0} sx={{
        ...
        <Box sx={{ pb: 1, flexShrink: 0 }}>
          <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1 }}>
            {isMdUp && (
              <Stack direction="row" spacing={1} alignItems="center">
                <Typography variant="h5" fontWeight={700}>Technical Leads</Typography>
          ...
          {/* Filters — Person / Company / Location / Lead Type + Date range */}

...
// --- MAIN COMPONENT ---
const TrafoAssociatesMyactivity = () => {
  ...
  const isMarketingHead = authRole === 'Trafo-Associates-Head';
  ...
  const [scheduleReason, setScheduleReason] = useState('Trafo Associates Visit Rescheduled.'); 
  ...
  const fetchAllData = async () => {
    ...
    try {
      const [unSchedRes, schedRes, compRes, statusRes] = await Promise.all([
        fetch(`${API_BASE_URL}/api/taleads/unscheduled-leads`, { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch(`${API_BASE_URL}/api/taleads/scheduled-visits`, { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch(`${API_BASE_URL}/api/taleads/completed-visits`, { headers: { 'Authorization': `Bearer ${token}` } }),
  ...
  const handleConfirmSchedule = async () => {
    ...
    try {
      ...
      if (isRescheduleMode) {
        // Hit Reschedule API
        apiUrl = `${API_BASE_URL}/api/taleads/reschedule-visit`;
        payload = {
        ...
        apiUrl = `${API_BASE_URL}/api/taleads/assign`;
        payload = {
          lead_id: selectedLead.lead_id,
  ...
  const handleStartVisit = async (lead) => {
    try {
      ...
      const visitPromise = fetch(`${API_BASE_URL}/api/taleads/visit-status`, {
        method: 'PATCH',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
  ...
  const submitEndVisit = async () => {
    ...
    try {
      ...
      promises.push(
        fetch(`${API_BASE_URL}/api/taleads/visit-status`, {
          method: 'PATCH',
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      // 2. Change Lead Stage
      promises.push(
        fetch(`${API_BASE_URL}/api/taleads/change-stage`, {
          method: 'PATCH',
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({
            lead_id: endVisitLead.lead_id,
      ...
      setTimeout(async () => {
        try {
          ...
          const notificationPayload = {
            ...
            message: isSolutions 
              ? `New lead transferred from Trafo Associates for solutions. Lead ID: ${endVisitLead.lead_id} (${endVisitLead.company_name || endVisitLead.lead_name})`
              : `Trafo Associates Lead Transferred to ${deptName}. Lead ID: ${endVisitLead.lead_id} (${endVisitLead.company_name || endVisitLead.lead_name}) by ${currentUser?.username || 'Employee'}.`
          };
...
export default TrafoAssociatesMyactivity;
...
const SilverlineAssociatesDashboard = () => {
  ...
  const isHead = authRole === 'Silverline-Associates-Head';
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      // 1. Fetch Hot Leads
      try {
        const res = await fetch(`${API_BASE_URL}/api/saleads/hot-leads`, { headers });
        if (res.ok) {
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/api/saleads/employees-revenue`, { headers });
        if (res.ok) {
          const result = await res.json();
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/api/saleads/new-assigned-summary`, { headers });
        if (res.ok) {
          const result = await res.json();
          if (isHead) {
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/api/saleads/sales-funnel`, { headers });
        if (res.ok) {
          const result = await res.json();
...
export default SilverlineAssociatesDashboard;